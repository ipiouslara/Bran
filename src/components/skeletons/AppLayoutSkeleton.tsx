...
export const AppLayoutSkeleton: React.FC<AppLayoutSkeletonProps> = ({
  ...
  return (
    <div className={`min-h-screen flex ${bgPage} text-white font-sans transition-colors duration-150`}>
      ...
      <aside className={`w-14 h-screen sticky top-0 border-r ${sidebarBg} flex flex-col justify-between p-2 shrink-0`}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Header Skeleton */}
        <header className={`h-14 px-6 border-b ${headerBg} flex items-center justify-between shrink-0`}>
        ...
        <main className="flex-1 p-6 space-y-6 overflow-y-auto">
          <PageSkeleton theme={theme} activePage={activePage} />
        </main>
      </div>
    </div>
  );
